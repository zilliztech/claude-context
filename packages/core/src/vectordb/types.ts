...
// New interface for hybrid search documents with multiple vector fields
export interface HybridVectorDocument {
    id: string;
    content: string; // Full text content for BM25 and storage
    vector: number[]; // Dense vector embedding
    sparse_vector?: number[]; // Sparse vector embedding (auto-generated by BM25 function)
    relativePath: string;
    startLine: number;
    endLine: number;
    fileExtension: string;
...
export interface VectorDatabase {
     ...
     * @param collectionName Collection name
     * @param documents Hybrid document array
     */
    insertHybrid(collectionName: string, documents: HybridVectorDocument[]): Promise<void>;
